...
const runExeApplyCommand = (status: number) => {
  const temporaryHome = mkdtempSync(join(tmpdir(), "exe-apply-command-"))
  try {
    mkdirSync(join(temporaryHome, ".dotfiles", "tasks", "dotfiles"), {recursive:true})
    writeFileSync(join(temporaryHome, ".dotfiles", "tasks", "dotfiles", "update.ts"), "")
    const result = Bun.spawnSync(["/bin/sh", "-c", `mise() { printf '%s\\n' "$*"; return ${status}; }; ${buildExeApplyCommand("core")}`], {
      env: { ...process.env, HOME: temporaryHome }, stdout: "pipe", stderr: "pipe"
    })
    return { exitCode: result.exitCode, trace: result.stdout.toString().trim().split("\n") }
  } finally {
...
test("applies and checks bootstrap postconditions with the complete Exe environment", async () => {
  ...
  expect(await Effect.runPromise(recording.commands)).toContainEqual({
    ...
    args: [
      ...EXE_DIRECT_SSH_ARGUMENTS,
      "exedev@work.exe.xyz",
      expect.stringContaining("'mise' '-E' 'linux' '-E' 'exe' 'run' 'dotfiles:pull'")
    ],
    cwd: expect.any(String)
  })
})

test("delegates a remote update once with the complete profile", () => {
  expect(buildExeApplyCommand("full")).toContain("'mise' '-E' 'linux' '-E' 'exe' '-E' 'full' 'run' 'dotfiles:pull'")
  for (const status of [0, 7]) {
    expect(runExeApplyCommand(status)).toEqual({ exitCode: status, trace: ["-E linux -E exe run dotfiles:pull"] })
  }
})
