...
export const waitForSshWithin = <E, R>(
  ...
  timeout: Duration.Input = "5 minutes",
  ...
  Effect.timeoutOrElse({
    ...
    orElse: () => Effect.fail(new MachineProviderFailure({
      ...
      detail: "SSH did not become ready within five minutes."
    }))
  })
)
...
export const ExeMachineProviderLayer = Layer.effect(MachineProvider, Effect.gen(function*() {
  ...
  const waitForSsh = (name: string) => waitForSshWithin(waitAttempt(name)).pipe(
    Effect.mapError((cause) => cause instanceof MachineProviderFailure && cause.detail.includes("five minutes")
      ? cause
      : failure("SSH readiness", "SSH did not become ready within five minutes.", cause))
  )
  const bootstrap = (name: string, profile: MachineProfile) => {
    const remoteEnv = profile === "full" ? "linux,full" : "linux"
    return run("bootstrap", "mise", ["bootstrap", "remote", "--host", hostFor(name), "--remote-env", remoteEnv, "--yes", "--locked", "--fail-fast"]).pipe(Effect.asVoid)
  }
  const remoteTask = (operation: string, name: string, command: string) => run(operation, "ssh", [hostFor(name), command]).pipe(Effect.asVoid)
  return MachineProvider.of({
    ...
    waitForSsh,
    bootstrap,
    apply: (name, profile) => remoteTask("apply", name, `cd "$HOME/.dotfiles" && mise run dotfiles:pull && mise run machine:apply${profile === "full" ? " full" : ""}`),
