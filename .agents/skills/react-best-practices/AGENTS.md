...
Comprehensive performance optimization guide for React applications, designed for AI agents and LLMs. Contains rules across 8 categories, prioritized by impact from critical (eliminating waterfalls, reducing bundle size) to incremental (advanced patterns). Each rule includes detailed explanations, real-world examples comparing incorrect vs. correct implementations, and specific impact metrics to guide automated refactoring and code generation.
...
3. [Server-Side Performance](#3-server-side-performance) — **HIGH**
   ...
   - 3.2 [Hoist Static I/O to Module Level](#32-hoist-static-io-to-module-level)
4. [Client-Side Performance](#4-client-side-performance) — **MEDIUM-HIGH**
   - 4.1 [Use React Query for Automatic Deduplication](#41-use-react-query-for-automatic-deduplication)
   - 4.2 [Deduplicate Global Event Listeners](#42-deduplicate-global-event-listeners)
   ...
   - 4.5 [Use useSyncExternalStore for External Subscriptions](#45-use-usesyncexternalstore-for-external-subscriptions)
5. [Re-render Optimization](#5-re-render-optimization) — **MEDIUM**
   ...
   - 5.16 [Don't Reset State in Effects on Prop Changes](#516-dont-reset-state-in-effects-on-prop-changes)
   - 5.17 [Reset State with Key, Not Effects](#517-reset-state-with-key-not-effects)
   - 5.18 [Don't Chain Effects via State](#518-dont-chain-effects-via-state)
   - 5.19 [Notify Parent in Event Handler, Not Effect](#519-notify-parent-in-event-handler-not-effect)
...
8. [Advanced Patterns](#8-advanced-patterns) — **LOW**
   ...
   - 8.4 [Use Callback Refs for DOM Measurement](#84-use-callback-refs-for-dom-measurement)
...
// Correct: checks existence first, then fetches in parallel
async function updateResource(resourceId: string, userId: string) {
  const resource = await getResource(resourceId)
...
In API routes and server functions, start independent operations immediately, even if you don't await them yet.
...
> **React Server Components only.** Async components (`async function Component()`) are only supported in RSC environments (Next.js App Router, etc.). In client-side SPAs, use `use()` with Suspense instead.
...
The `typeof window !== 'undefined'` check prevents this import from executing during SSR. Note: the module is still included in the bundle — this guard only prevents runtime execution on the server.
...
const Analytics = lazy(() =>
  import('analytics-lib').then(m => ({ default: m.Analytics }))
)
...
The `typeof window !== 'undefined'` check prevents these imports from executing during SSR. Note: the module is still included in the bundle — this guard only prevents runtime execution on the server.
...
For data shared across sequential requests (user clicks button A then button B), use an LRU cache.
...
### 3.2 Hoist Static I/O to Module Level
...
### 4.1 Use React Query for Automatic Deduplication
...
### 4.5 Use useSyncExternalStore for External Subscriptions

**Impact: MEDIUM (correct concurrent-safe subscription)**
...
`useSyncExternalStore` is concurrent-safe and avoids tearing (showing inconsistent state across the UI during concurrent renders).

**Common use cases:** Browser APIs (`navigator.onLine`, `matchMedia`, `localStorage`), third-party state libraries without React bindings, WebSocket connection status.
...
**Note:** If your project has [React Compiler](https://react.dev/learn/react-compiler) enabled, manual memoization with `memo()` is not necessary. The compiler automatically optimizes re-renders.
...
**Note:** If your project has [React Compiler](https://react.dev/learn/react-compiler) enabled, the compiler automatically optimizes re-renders, but `useDeferredValue` still provides value for intentionally deferring expensive derived renders.
...
### 5.16 Don't Reset State in Effects on Prop Changes

**Impact: MEDIUM (avoids extra renders with stale values)**
...
- **Reset all state with `key`** — when *all* state should reset (see 5.17)
...
### 5.17 Reset State with Key, Not Effects

**Impact: MEDIUM (avoids stale state and extra renders)**
...
The component renders with the stale comment first, then the effect clears it causing a second render.
...
### 5.18 Don't Chain Effects via State

**Impact: MEDIUM (eliminates cascading re-renders)**
...
React batches all `setState` calls within an event handler into a single render.
...
### 5.19 Notify Parent in Event Handler, Not Effect

**Impact: MEDIUM (single render pass instead of two)**
...
React batches both updates (local + parent) into one render. Even better — lift state to the parent entirely when two components need to stay in sync.
...
**Incorrect: recreates static element descriptor every render**

```tsx
function Container() {
  // This JSX creates a new element object on every render
...
The inline script executes synchronously and updates the DOM before React hydrates. `suppressHydrationWarning` tells React to expect the mismatch on this element (the script sets className that differs from the server-rendered HTML).
...
React DOM provides APIs to hint the browser about resources it will need, starting resource loading early.
...
**Caveat:** Modern JS engines (V8, SpiderMonkey) optimize property access via inline caches, so this matters mainly for extremely hot paths with millions of iterations. Prefer readability over this optimization in most code.
...
```typescript
// Module-level cache with size limit to prevent memory leaks
...
**Impact: MEDIUM (avoids expensive operations when lengths differ)**
...
### 8.4 Use Callback Refs for DOM Measurement

**Impact: LOW (reliable DOM side effects on mount)**
