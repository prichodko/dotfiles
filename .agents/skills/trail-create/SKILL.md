...
For the same task and trail, validate, commit, and push each later related change after the trail is created or confirmed. Do not require another `$trail-create` invocation for that scope. If the trail is created before the current requested work is complete, ask once before applying this rule to the remaining work, unless that continuation is already authorized. Do not carry this authorization to another task or trail. An explicit instruction to keep work uncommitted or not push overrides this rule.
